...
public partial class Baskets
{
    ...
    {
        public record Command(int CustomerId, int ProductId, int Quantity);

        public record Response(int ProductId, int Quantity);
        ...
        public class Handler
        {
            ...
            public async Task<Response> HandleAsync(Command command, CancellationToken cancellationToken)
            {
                var item = await _db.Items.AsNoTracking().FirstOrDefaultAsync(
                    ...
                    cancellationToken: cancellationToken) ?? throw new BasketItemNotFoundException(command.ProductId);
